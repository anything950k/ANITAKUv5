...
export const MangaReaderModal: React.FC = () => {
  ...
  // Novel dynamic canvas background & text styling
  const novelColorStyles = useMemo(() => {
    if (novelSettings.pageColor === 'sepia') {
      return { bg: '#F5EEDC', text: '#3d2e1e', border: 'transparent' };
    }
    if (novelSettings.pageColor === 'custom') {
      return {
    ...
    if (novelSettings.pageColor === 'amoled') {
      return { bg: '#000000', text: '#ffffff', border: 'transparent' };
    }
    if (novelSettings.pageColor === 'dark') {
      return { bg: '#101017', text: '#e2e4ee', border: 'transparent' };
    }
    // If 'paper' or matching app settings:
    if (readerBg === 'black' || settings.readerBackground === 'Black') {
      return { bg: '#000000', text: '#ffffff', border: 'transparent' };
    }
    if (readerBg === 'dark' || settings.readerBackground === 'Dark Gray' || settings.readerBackground === 'Dark') {
      return { bg: '#101017', text: '#e2e4ee', border: 'transparent' };
    }
    if (settings.readerBackground === 'White' || readerBg === 'light') {
      return { bg: '#FFFFFF', text: '#1a1a24', border: 'transparent' };
    }
    return { bg: '#FFFFFF', text: '#1a1a24', border: 'transparent' };
  }, [novelSettings.pageColor, novelSettings.customColors, readerBg, settings.readerBackground]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`w-full h-full no-scrollbar cursor-pointer bg-[#000000] ${
      ...
      >
        {isLoading ? (
          <div className="fixed inset-0 z-40 bg-[#000000] flex flex-col items-center justify-center select-none">
            <div className="w-8 h-8 rounded-full bg-[#9d62f8] animate-dot-pulse shrink-0" />
            <p className="text-[13px] text-white/70 font-normal tracking-wide mt-3.5">
              Loading page...
            </p>
            ...
            <div className="flex items-center gap-3 px-1">
              ...
              <div className="relative flex-1 flex items-center h-12 sm:h-14 select-none mx-1 group touch-none">
                ...
                <div
                  ...
                  } rounded-l-full rounded-r-[3.5px] overflow-hidden flex items-center z-[2]`}
                  style={{
                ...
                {/* 3. Dots behavior: For Novel -> Single End Dot Indicator exactly like Novel Setting Sheet; For Manga -> Fixed Stationary Dots Across the Entire Track */}
                {isNovel ? (
                  <div
                ...
                {/* 4. Vertical Indicator Line | (Taller prominent rounded pill with increased vertical height) */}
                <div
                ...
                >
                  <div
                    className={`w-[3.5px] sm:w-[4px] h-[46px] sm:h-[50px] ${
                      isNovel ? 'bg-[#b876fc]' : 'bg-[#a855f7]'
